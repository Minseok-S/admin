...
import { InteractiveInfoCardData } from "../types/home";

/**
 * 인터랙티브 정보 카드 컴포넌트
 *
 * 애니메이션 효과가 적용된 카드로, 마우스 호버 시 상승 및 확대 효과를 제공합니다.
 * 모바일에서는 호버 효과 대신 탭 효과가 적용됩니다.
 *
 * @param {InteractiveInfoCardData} props.card - 카드에 표시할 데이터 객체
 * @param {number} props.index - 카드의 표시 순서 (1부터 시작하는 숫자로 표시됨)
 * @returns {JSX.Element} 인터랙티브 정보 카드 컴포넌트
 */

export const InteractiveInfoCard = (
  card: InteractiveInfoCardData,
  index: number
) => {
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative h-60 sm:h-64 md:h-72 rounded-xl sm:rounded-2xl overflow-hidden mb-5 sm:mb-6 transform transition-transform duration-700 group-hover:scale-[1.02] shadow-lg">
        <Image
          src={card.image}
          alt={card.title}
          fill
